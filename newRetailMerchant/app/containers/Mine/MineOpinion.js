...
class MineOpinion extends Component {
  constructor (props) {
    super (props);
    ...
    // this._getCommentStoreConfig = this._getCommentStoreConfig.bind(this); //显示人均消费
    // this._getCommentStorePro = this._getCommentStorePro.bind(this);
    // this._getChildData = this._getChildData.bind(this);
    // this._addCameraImage = this._addCameraImage.bind(this);
  ...
  _renderImage (image) {
    return (
      <View style={{width: screenWidth / 7, marginRight: 6, marginBottom: 6}}>
        <Image
          style={{
            width: screenWidth / 7,
  ...
  _renderSection (str) {
    return (
      <View style={{width: screenWidth, height: 35, justifyContent: 'center'}}>
        <Text
          style={{
            left: 15,
            color: '#4a4a4a',
  ...
  _commitButton = () => {
    this.state.images ? this._getUploadImg () : this._storeOpinionSub ();
  };
  ...
  _storeOpinionSub () {
    ...
    if (this.state.styleStr == '' || this.state.opinionContext == '') {
        ...
        <ScrollView style={{flex: 1}}>
          ...
          >
            ...
            >
              <Text style={{fontSize: 12, color: '#9b9b9b'}}>
                {this.state.opinionContext.length}
              </Text>
              <Text style={{fontSize: 12, color: '#9b9b9b'}}>/500</Text>
            </View>
          </View>
            ...
            >
              {isNotEmptyArray (this.state.images)
                ? this.state.images.map (i => (
                    <View>{this._renderImage (i)}</View>
                  ))
                : this.state.selectImage !== null
                    ? <View>{this._renderImage (this.state.selectImage)}</View>
                    : <TouchableOpacity
                        onPress={() => {
          ...
          <Button
            ...
            style={[
              styles.buttonCommit,
              {
                backgroundColor: this.state.btnActive
...
export default connect (select) (MineOpinion);
