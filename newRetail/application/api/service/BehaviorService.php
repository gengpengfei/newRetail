...
use app\api\model\BehaviorActionModel;
use app\api\model\BehaviorLogModel;
use app\api\model\BehaviorModel;
use app\api\model\StoreOrderModel;
use app\api\model\UserActiveLogModel;
    ...
    protected function check($time){
        ...
        if($behaviorNum<=$this->behavior->behavior_times){
            ...
            if($this->behavior->order_times>0){
                ...
                $orderNum = StoreOrderModel::where('store_id='.$this->store_id.' and user_id=' . $this->userInfo->user_id .' and order_state="T03" and create_time>"'.$time.'"')->count();
