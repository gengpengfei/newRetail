...
use app\admin\service\ValidateService;
...
{
    public function __construct(ValidateService $validateService,Request $request,UserActionModel $userActionModel,ActionModel $actionModel)
    {
        //不验证是否登陆的控制器
        ...
        parent::__construct();
        if(empty(Session::get('admin_user_id')) && !in_array($request->controller(),$not_login)){
            $this->redirect('Admin/Login/index');
        }
        $validateService->validate($_POST);
        //获取用户权限，序列或者all
        if(!empty(Session::get('admin_user_id'))) {
            if (empty(Session::get("admin_action_list"))) {
                $admin_action_list = $userActionModel->where("admin_user_id=" . Session::get('admin_user_id'))->find();
                $admin_action_list = $admin_action_list->admin_action_list;
                Session::set("admin_action_list", $admin_action_list);
            } else {
                $admin_action_list = Session::get("admin_action_list");
            }
            //如果需要验证权限
            if (!in_array($request->controller(), $not_rule)) {
                if (!empty($admin_action_list)) {
                    ...
                    $this->error('系统异常，请重新登录', 'Admin/Login/index');
                }
            }
        }
    }
    ...
    public function setAdminUserLog($type='',$cont='',$table_name='',$table_id='')
    {
        ...
        $userLogModel->admin_user_id = Session::get('admin_user_id');
        $userLogModel->admin_nickname = Session::get('admin_nickname');
    ...
    public function setOrderLog($type='',$cont='',$order_id='',$table_name='',$table_id='')
    {
        ...
        $orderLogModel->admin_user_id = Session::get('admin_user_id');
    ...
    public function setOrderMessage($cont='',$order_id='')
    {
        ...
        $orderMessageModel->admin_user_id = Session::get('admin_user_id');
